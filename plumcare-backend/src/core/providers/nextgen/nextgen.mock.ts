...
export class NextGenMockProvider extends BaseEhrProvider {
  ...
  async getObservations(patientId: string, params?: PaginationParams): Promise<PaginatedResponse<Observation>> {
    ...
    // Get vitals from encounters for this patient and convert to FHIR Observations
    const patientIdStr = patientId.replace('nextgen-p-', '');
    const patientEncounters = mockDataStore
      .filter(d => d.patient.person_id === patientIdStr)
      .flatMap(d => d.encounters)
      .filter(e => e.vitals);

    const observations: Observation[] = patientEncounters.flatMap(encounter =>
      this.convertVitalsToObservations(encounter.vitals!, patientId, encounter)
    );
  ...
  // Helper: Convert NextGen vitals to FHIR Observations
  private convertVitalsToObservations(
    vitals: NextGenEncounter['vitals'],
    patientId: string,
    encounter: NextGenEncounter
  ): Observation[] {
    if (!vitals) return [];

    const observations: Observation[] = [];
    const vitalCodeMap: Record<string, { code: string; display: string; unit: string }> = {
      ...
      'pulse_rate': { code: '8867-4', display: 'Heart rate', unit: '/min' },
      'respiratory_rate': { code: '9279-1', display: 'Respiratory rate', unit: '/min' },
      'temperature_fahrenheit': { code: '8310-5', display: 'Body temperature', unit: '[degF]' },
      'oxygen_saturation': { code: '2708-6', display: 'Oxygen saturation', unit: '%' },
      'weight_lbs': { code: '29463-7', display: 'Body weight', unit: '[lb_av]' },
      'height_inches': { code: '8302-2', display: 'Body height', unit: '[in_i]' },
    ...
    for (const [key, codeInfo] of Object.entries(vitalCodeMap)) {
      ...
      if (value !== undefined && typeof value === 'number') {
        observations.push({
          ...
          identifier: [{
            system: 'http://plumcare.io/nextgen/observation',
            value: `${vitals.vitals_id}-${key}`,
          }],
          status: 'final',
          category: [{
            coding: [{
          ...
          effectiveDateTime: vitals.recorded_date || encounter.encounter_date,
          valueQuantity: {
            value,
            unit: codeInfo.unit,
  ...
  // Helper: Convert NextGen problem to FHIR Condition
  private convertProblemToCondition(problem: NextGenProblem, patientId: string): Condition {
    ...
    return {
      ...
      identifier: [{
        system: 'http://plumcare.io/nextgen/condition',
        value: problem.problem_id,
      }],
      clinicalStatus: statusMap[problem.status] || statusMap['Active'],
      code: {
        coding: [{
          system: problem.icd_version === 'ICD-10' ? 'http://hl7.org/fhir/sid/icd-10-cm' : 'http://hl7.org/fhir/sid/icd-9-cm',
      ...
      abatementDateTime: problem.resolution_date,
      recordedDate: problem.created_timestamp,
